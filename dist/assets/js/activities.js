...
function init() {
	...
	(0, _helpers.on)('click', _balajs2.default.one('#back-button'), function () {
		var route = {
			level: _state2.default.get('route').level - 1
		};
		_state2.default.routeTo(route);
	});
}
...
function setSelectedCategory(data) {
	_state2.default.routeTo({ level: 1, category: data.slug });
		...
		value: function navigate(route) {
			...
			switch (route.level) {
				case 0:
					break;
				case 1:
					hashString += route.category;break;
				case 2:
					hashString += route.category + '/' + route.activity;break;
			}

			// change hash value (generates new history record)
			_hasher2.default.setHash(hashString);
		}
	}, {
...
var state = {
	...
	routeTo: function routeTo(r) {
		var direction = this.store.route.level >= r.level ? '' : 'reverse';
